...
exports.getValidDistCerts = getValidDistCerts;
...
class CreateIosDist {
  ...
  async provideOrGenerate(ctx) {
    ...
    if (userProvided) {
      if (!ctx.hasAppleCtx()) {
        _chalk().default.yellow("WARNING! Unable to validate Distribution Certificate due to insufficient Apple Credentials. Please double check that you're uploading valid files for your app otherwise you may encounter strange errors!");
...
class CreateOrReuseDistributionCert {
  ...
  async _choosePreferredCreds(distCerts, appCredentials, experienceName) {
    // prefer the one that matches our experienceName
    const appCredentialsForExperience = appCredentials.filter(cred => cred.experienceName === experienceName);
  ...
  async open(ctx) {
    ...
    const autoselectedCertificate = await this._choosePreferredCreds(existingCertificates, ctx.ios.credentials.appCredentials, this._experienceName);
...
function getOptionsFromProjectContext(ctx, options = {}) {
...
async function getValidDistCerts(iosCredentials, appleCtx) {
  ...
  return await filterRevokedDistributionCerts(appleCtx, distCerts);
}
...
async function selectDistCertFromList(ctx, options = {}) {
  ...
  if (ctx.hasAppleCtx()) {
    validDistCerts = await filterRevokedDistributionCerts(ctx.appleCtx, distCerts);
  }
...
async function validateDistributionCertificate(appleContext, distributionCert) {
  ...
  const validDistributionCerts = await filterRevokedDistributionCerts(appleContext, [distributionCert]);
...
async function filterRevokedDistributionCerts(appleCtx, distributionCerts) {
