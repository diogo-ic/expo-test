...
async function selectPushKey(context, options = {}) {
  ...
  const pushKeys = _lodash().default.get(context, 'user.username') ? await chooseUnrevokedPushKey(context) : [];
  const choices = [...pushKeys]; // autoselect creds if we find valid ones

  if (pushKeys.length > 0 && !options.disableAutoSelectExisting) {
    const autoselectedPushkey = (0, _selectUtils().choosePreferredCreds)(appleContext, pushKeys);
  ...
  if (promptValue === 'GENERATE') {
    ...
    const pushKey = (await (0, _promptForCredentials().default)(appleContext, ['pushKey'])).credentials.pushKey;
...
async function validateUploadedPushKey(appleContext, pushKey) {
  ...
  const filteredFormattedPushKeyArray = await filterRevokedPushKeys(appleContext, formattedPushKeyArray);
...
async function chooseUnrevokedPushKey(context) {
  const appleContext = context.appleCtx;
  const pushKeysOnExpoServer = await _xdl().Credentials.Ios.getExistingPushKeys(context.user.username, appleContext.team.id, {
    provideFullPushKey: true
  });
